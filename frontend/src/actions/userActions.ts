...
export const googleLogin = (user: any) => async (dispatch: any) => {
  try {
    dispatch({
      type: USER_LOGIN_REQUEST,
    });
    ...
    const { data } = await axios.post("/api/users/googlelogin", user, config);
...
export const facebookLogin = (user: any) => async (dispatch: any) => {
  try {
    dispatch({
      type: USER_LOGIN_REQUEST,
    });
    ...
    const { data } = await axios.post("/api/users/facebooklogin", user, config);
